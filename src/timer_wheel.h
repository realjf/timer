...
namespace CTimer {
    ...
    public:
        TimerWheel();
        TimerWheel(int bitShift, int wheelSize);
    ...
    template <typename T>
    TimerWheel<T>::TimerWheel() : shiftBits_(kBitShift), wheelMask_(kWheelSize - 1) {
        // 初始化每个槽位为null
    ...
    template <typename T>
    TimerWheel<T>::TimerWheel(int bitShift, int wheelSize) : shiftBits_(bitShift), wheelMask_(wheelSize - 1) {
        // 初始化每个槽位为null
